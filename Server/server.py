...
#         return {"hello": "{content}".format(content=content)}
...
class Emotion(Resource):
    def post(self): 
        ...
        # 영화 데이터 저장
        diary['movies'] = movie_data.sort_values(by='cos_sim', ascending=False).head(5).replace(np.nan, '', regex=True).to_dict('records')

        print(content)
        print(json.dumps(diary, indent=4, ensure_ascii=False))
        print("요청 처리완료")
